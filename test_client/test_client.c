...
struct gsm_cell_scan gsm_cell = {603,1,14962,1551480950,16101,-128,SKY_BEACON_GSM};
...
*/
	...
	Sky_finalize_t finalize = sky_finalize_request(ctx, &sky_errno, &prequest, &request_size,
                                 &loc, &response_size);
	...
	int32_t rc = send_request((char*) prequest, (int) request_size,
								response, response_size, config.server, config.port);
	if (rc > 0)
