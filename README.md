...
* Library is more compact and configurable to include only the required features.
...
| Declaration                 | Recommended Value                           |
|:----------------------------|:--------------------------------------------|
|                             |                                             |
| `CACHE_SIZE`                |1                                            |
| `SKY_MAX_DL_APP_DATA`       |100 (maximum length of downlink data)        |
| `SKY_TBR_DEVICE_ID`         |true                                         |
| `SKY_LOGGING`               |true                                         |
| `SKY_LOG_LENGTH`            |120 (maximum char on a log line)             |
| `SANITY_CHECKS`             |true (set to false to exclude sanity checks) | 
| `SKY_EXCLUDE_WIFI_SUPPORT`  |false (set to true to exclude Wi-Fi support) |
| `SKY_EXCLUDE_CELL_SUPPORT`  |false (set to true to exclude Cell Support   | 
| `SKY_EXCLUDE_GNSS_SUPPORT`  |false (set to true to exclude GNSS Support   |
...
* `CACHE_SIZE` typically set to '1' meaning that there is one available cache entry reserved. Setting `CACHE_SIZE` to 0
  will disable the cache. Values of `CACHE_SIZE` above `1` may provide some improvements to cache performance, however
  should you find the performance of `CACHE_SIZE` `1` suboptimal, you should contact your Skyhook representative.
...
* `SKY_LOGGING` controls whether debug information is generated by the library. By default, it
  ...
  `SKY_LOG_LENGTH` can be used to adjust the maximum line length of logging, if required.
* `SANITY_CHECKS` may be used to minimize the size of the library, however, when possible, it is recommended that sanity
  checks be included during the application development process.
* `SKY_EXCLUDE_WIFI_SUPPORT` may be set to true if no Wi-Fi scan data will be added to a request. This reduces the size
  of the library code.
* `SKY_EXCLUDE_CELL_SUPPORT` may be set to true if no CELL scan data will be added to a request. This reduces the size
  of the library code.
* `SKY_EXCLUDE_GNSS_SUPPORT` may be set to true if no GNSS scan data will be added to a request. This reduces the size
  of the library code.
