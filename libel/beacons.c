...
static Sky_status_t filter_by_rssi(Sky_ctx_t *ctx)
{
    ...
    if (ctx->beacon[ctx->ap_len - 1].ap.rssi - ctx->beacon[0].ap.rssi < ctx->ap_len) {
    ...
    LOGFMT(ctx, SKY_LOG_LEVEL_DEBUG, "range: %d band: %.2f",
        (ctx->beacon[ctx->ap_len - 1].ap.rssi - ctx->beacon[0].ap.rssi), band_range)
    ...
    for (i = 1, reject = ctx->ap_len / 2, worst = 0; i < ctx->ap_len - 2; i++) {
        if (fabs(ctx->beacon[i].ap.rssi - ideal_rssi[i]) > worst) {
            ...
            reject = i;
            LOGFMT(ctx, SKY_LOG_LEVEL_DEBUG, "reject: %d, ideal %.2f worst %.2f", i, ideal_rssi[i],
                worst)
        }
    }
...
static Sky_status_t filter_virtual_aps(Sky_ctx_t *ctx)
{
    ...
    LOGFMT(ctx, SKY_LOG_LEVEL_DEBUG, "ap_len: %d of %d APs", (int)ctx->ap_len, (int)ctx->len)
    dump_workspace(ctx);
...
int find_best_match(Sky_ctx_t *ctx, bool put)
{
    ...
    for (i = 0; i < CACHE_SIZE; i++) {
        ...
        LOGFMT(ctx, SKY_LOG_LEVEL_DEBUG, "cache: %d: score %.2f (%d/%d)", i, ratio[i] * 100,
            score[i], ctx->len + ctx->cache->cacheline[i].len - score[i])
    ...
    if (!put) {
        if (ctx->len <= CACHE_BEACON_THRESHOLD && bestscore == ctx->len) {
            LOGFMT(ctx, SKY_LOG_LEVEL_DEBUG, "Only %d beacons; pick cache %d of 0..%d score %.2f",
                ctx->len, bestc, CACHE_SIZE - 1, ratio[bestc] * 100)
            return bestc;
        } else if (ctx->len > CACHE_BEACON_THRESHOLD && bestratio * 100 > CACHE_MATCH_THRESHOLD) {
            LOGFMT(ctx, SKY_LOG_LEVEL_DEBUG, "location in cache, pick cache %d of 0..%d score %.2f",
                bestc, CACHE_SIZE - 1, score[bestc] * 100)
            return bestc;
        }
    } else if (bestc >= 0) { /* match is for put */
        LOGFMT(ctx, SKY_LOG_LEVEL_DEBUG, "save location in best cache, %d of 0..%d score %.2f",
            bestc, CACHE_SIZE - 1, score[bestc] * 100);
        return bestc;
    }
